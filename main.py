...
NUTRITIONIST_KEYBOARD = ReplyKeyboardMarkup([
    ["Составить меню на день 🍽️"],
    ["Анализ продукта по названию 🔍"],
    ["Задать вопрос нутрициологу ❓"],
    ["⬅️ Назад к выбору специалиста"]
], resize_keyboard=True)

FITNESS_TRAINER_KEYBOARD = ReplyKeyboardMarkup([
    ["Составить план тренировок 💪"],
    ["Вопрос по упражнению 🏋️"],
    ["Совет по восстановлению 🧘"],
    ["⬅️ Назад к выбору специалиста"]
], resize_keyboard=True)

PSYCHOTHERAPIST_KEYBOARD = ReplyKeyboardMarkup([
    ["Дневник настроения 🧠"],
    ["Техника дыхания для успокоения 🌬️"],
    ["Задать вопрос психотерапевту ❓"],
    ["⬅️ Назад к выбору специалиста"]
], resize_keyboard=True)

GENERAL_SPECIALIST_KEYBOARD = ReplyKeyboardMarkup([
    ["Задать вопрос специалисту ❓"],
    ["⬅️ Назад к выбору специалиста"]
], resize_keyboard=True)

DIARIES_KEYBOARD = ReplyKeyboardMarkup([
    ["Дневник питания 🥕", "Дневник тренировок 🏋️"],
    ["Дневник здоровья ❤️‍🩹", "Дневник настроения 📊"],
...
MOOD_KEYBOARD = ReplyKeyboardMarkup([
    ...
    ["Нормальное 😐"],
    ["Плохое 😕", "Очень плохое 😔"],
    ["Посмотреть дневник настроения 📊", "⬅️ Назад к психотерапевту"]
], resize_keyboard=True)
...
# --- Функции Специалистов (Питание, Тренировки) ---
async def create_personalized_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    menu_prompt = (
        ...
        f"Для каждого приема пищи (завтрак, обед, ужин) в обоих меню, укажи:\n"
        "- 🍳/🥗/🍲 Название блюда\n"
...
async def create_workout_plan_final(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    workout_prompt = (
        f"Ты — {role_prompt}. Создай подробный план тренировок на неделю (3 дня), используя данные пользователя.\n"
        f"Тренировки будут проходить '{workout_location}'.\n"
        f"{get_personal_prompt(profile, data.get('first_name'))}\n"
        f"Для каждого тренировочного дня:\n"
        "- 🗓️ Тип тренировки (например, 'Круговая ВИИТ', 'Силовая на все тело').\n"
...
async def analyze_product_by_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text("Эта функция в разработке. Скоро я смогу анализировать пищевую ценность любого продукта по его названию! 🏗️", reply_markup=NUTRITIONIST_KEYBOARD)

async def handle_exercise_question(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text("Эта функция в разработке. Скоро вы сможете задать мне вопрос о технике любого упражнения! 🏗️", reply_markup=FITNESS_TRAINER_KEYBOARD)

async def handle_breathing_technique(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text("Эта функция в разработке. Скоро я смогу проводить для вас сессии дыхательных практик! 🌬️", reply_markup=PSYCHOTHERAPIST_KEYBOARD)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if "задать вопрос" in message_text.lower():
        ...
        context.user_data['context_state'] = 'awaiting_question_for_specialist'
        return
